...
void runAllBenchmarks(const BenchmarkConfig& config) {
    ...
    for (int size : config.matrix_sizes) {
        ...
        for (int threads : config.thread_counts) {
            Timer timer;
            auto result = CPUMatrixOperations::multiplyMatricesBlocked(A, B, M, N, K, 32, threads);
    ...
    for (int size : config.matrix_sizes) {
        ...
        for (size_t workgroup : config.workgroup_sizes) {
            try {
                Timer timer;
                auto result = gpu_ops.multiplyMatricesBlocked(A, B, M, N, K, workgroup);
                double time = timer.elapsed();
    ...
    for (int size : config.matrix_sizes) {
        ...
        // CPU time
        Timer timer;
        auto cpu_result = CPUMatrixOperations::multiplyMatricesBlocked(A, B, M, N, K, 32, cpu_threads);
        double cpu_time = timer.elapsed();
        
        // GPU time
        timer = Timer();
        auto gpu_result = gpu_ops.multiplyMatricesBlocked(A, B, M, N, K, gpu_workgroup);
